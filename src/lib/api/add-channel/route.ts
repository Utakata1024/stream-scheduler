...
export async function POST(request: Request) {
  try {
    ...
    if (!channelInput || !platform) {
      return NextResponse.json({ error: 'チャンネル情報またはプラットフォームが指定されていません。' }, { status: 400 });
    }
    ...
    if (platform === 'youtube') {
      if (!YOUTUBE_API_KEY) throw new Error("YouTube APIキーがサーバーに設定されていません");
      channelDetails = await fetchYoutubeChannelDetails(channelInput, YOUTUBE_API_KEY);
    } else if (platform === 'twitch') {
      if (!TWITCH_CLIENT_ID || !TWITCH_CLIENT_SECRET) throw new Error("Twitchの認証情報がサーバーに設定されていません");
      ...
      const twitchUser = await fetchUserByLogin(channelInput, twitchAccessToken, TWITCH_CLIENT_ID);
      if (twitchUser) {
        channelDetails = {
    ...
    if (!channelDetails) {
      return NextResponse.json({ error: '指定されたチャンネルが見つかりません。' }, { status: 404 });
    }
    ...
    const errorMessage = error instanceof Error ? error.message : "サーバーで予期せぬエラーが発生しました。";
